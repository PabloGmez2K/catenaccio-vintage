...
- [ ] **S023C — CONTROLLED_TERM_CREATION** — **NEXT / SIGUIENTE BLOQUE RECOMENDADO.** Crear término faltante desde Studio (`POST .../terms`) con dedupe + write-through a caché. Agente Sonnet **ASK→CODE** (WC write). Gate: `ACF_CONFIG` + dedupe. Test Pablo: crear equipo nuevo desde Studio, verificar en WP Admin; reintento no duplica.
