...
- [x] **THEME_SHADOW_VISUAL_BLOCKERS_FIX** — fix quirúrgico de 5 blockers visuales detectados por Pablo en preview manual: mini-cart ahora trigger+dropdown (no inline), toggle off-canvas oculto en desktop con `!important`, off-canvas con `overflow-x:hidden` + word-break, CSS defensivo filtros sidebar, CSS defensivo toolbar. 3 archivos modificados (header.php, cv-a0.css, cv-a0.js). Ver `docs/operations/THEME_SHADOW_VISUAL_BLOCKERS_FIX.md`. (Sesión 016, 2026-06-27)
