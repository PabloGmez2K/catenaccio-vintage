...
- [~] **S023A — WC_TAXONOMY_CATEGORY_READ_SYNC** — **READY_FOR_PABLO_SQL_APPLY (2026-06-29, Codex).** Implementada caché Supabase (`wc_taxonomies`/`wc_terms`/`wc_categories`) + endpoint autenticado `POST /inventory/sync` + verificador SQL. WC sigue read-only por diseño (solo GET cuando Pablo dispare sync); no se tocó bridge/form/`wc-terms-mvp.ts`. Gate `DATA_LAYER_MAPPING`: PASS. Pendiente test Pablo: aplicar SQL, disparar sync, ejecutar `scripts/studio/verify_wc_term_cache.sql`, confirmar Real Madrid→70 / FC Barcelona→170 / 2014-15→139, counts cache == WC y `wc_post_called=false`. Cierre funcional esperado: `APPROVE_READY_FOR_S023B`.
- [ ] **S023B — TERM_CACHE_BACKED_OPTIONS** — form + bridge resuelven términos contra la caché; deprecar `wc-terms-mvp.ts` (conservar opciones de presentación, eliminar term IDs manuales). Agente Sonnet. Gate: `PRODUCT_REFERENCE_DIFF`. Test Pablo: borrador de equipo que existía en WC pero tenía `termId:''` resuelve solo.
- [ ] **S023C — CONTROLLED_TERM_CREATION** — crear término faltante desde Studio (`POST .../terms`) con dedupe + write-through a caché. Agente Sonnet **ASK→CODE** (WC write). Gate: `ACF_CONFIG` + dedupe. Test Pablo: crear equipo nuevo desde Studio, verificar en WP Admin; reintento no duplica.
- [ ] **S023D — PLAYER_TERM_RESOLUTION** — aplicar caché + creación a `pa_jugador`; resolver Rivaldo. Agente Sonnet. Gate: `FULL_FIXTURE` con jugador. Cierra el fallo diferido de S022C.8: Jugador aparece seleccionado en WP Admin.
