...
- [ ] **S022A — STUDIO_CREATE_ITEM_FORM** — READY_FOR_PABLO_VALIDATION (2026-06-28, Sonnet). Formulario `/inventory/new` implementado: Server Action en `actions.ts`, `ItemForm` client component, ruta `new/page.tsx`. Escribe `inventory_items` + `football_shirt_details` con owner_id + workspace_id. `/inventory` con botón "Nueva camiseta". `/inventory/[id]` con precio_objetivo, jugador, notas. typecheck PASS, build PASS, lint PASS. Falta: Pablo crea ≥1 camiseta real local y confirma. Ver `docs/studio/STUDIO_CREATE_ITEM_FORM_RESULT.md`.
- [ ] **S022B — STUDIO_AI_SUGGESTIONS_SHADOW** — "Sugerir con Claude" (Anthropic API, server-side only) → `ai_suggestions` versionada → Pablo aprueba/edita. SHADOW_FIRST: nunca fluye sola a WC. Sin WC, sin deploy. Parada: Pablo aprueba una sugerencia para la camiseta de S022A. Veredicto esperado: `APPROVE_READY_FOR_S022C`. (Sonnet) — _fusionable con S022A si el alcance del formulario resulta pequeño (ambas locales)._
