...
- [ ] **S024A — TAXONOMY_INPUT_STANDARDIZATION** — **READY_FOR_PABLO_TAXONOMY_UX_TEST (2026-07-01, Opus 4.8).** Estandarizados los 4 campos de taxonomía creable (Liga/Equipo/Año/Jugador) en el formulario: sugerencias desde la caché real `wc_terms` (S023A) en lugar de listas estáticas; estado visible por campo (✓ Existe en Woo · ID / Nuevo término / Sin caché); botón "Crear término en Woo" **solo cuando el término no existe** y hay texto suficiente; entrada manual siempre libre; las listas estáticas de `wc-terms-mvp.ts` quedan como fallback visual solo si la caché está vacía. Jugador deja de depender del autocompletado del navegador (ahora muestra los términos cacheados, incl. Rivaldo/Ronaldinho). Nuevo helper `studio/lib/wc/term-options.ts` + componente reutilizable `studio/components/TaxonomyTermField.tsx`; `actions.ts`/bridge sin cambios (resolución/guardado intactos). typecheck/build/lint/diff-check PASS, secret scan CLEAN; sin SQL, sin Woo por agente, sin publicación. Difiere `FUTURE_TAXONOMY_SMART_SUGGESTIONS` (grafo/contexto). Ver `docs/studio/STUDIO_TAXONOMY_INPUT_STANDARDIZATION_RESULT.md`. Pendiente: Pablo prueba en local y confirma `PABLO_TAXONOMY_UX_OK` antes de abrir S024.
- [ ] **S024–S030 — SUITE ROADMAP** — Siguiente bloque recomendado: S024 completeness/preflight, no abierto en S023E.CLOSE. Después: imágenes (S025), stock operativo (S026), Rank Math/SEO (S027), landings (S028), drift detection (S029), publicación controlada (S030). Una tarea por sesión, gates por fase. Ver `docs/studio/STUDIO_ROADMAP_S023_S030.md §3` y matriz de gates `docs/studio/STUDIO_SESSION_GATES.md §7`.
