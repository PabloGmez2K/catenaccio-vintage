...
- [x] **S023C — CONTROLLED_TERM_CREATION** — **READY_FOR_PABLO_TERM_CREATION_TEST (2026-06-30, Sonnet).** Implementado `studio/lib/wc/term-create.ts` (`createControlledTerm`): dedupe en 2 capas (caché `wc_terms` → `GET` en vivo a Woo) antes de `POST .../products/attributes/{id}/terms`; write-through inmediato a `wc_terms` con el ID real devuelto por Woo (`source='studio_created'`). Acción explícita vía `studio/app/inventory/term-actions.ts` + botón `TermCreateButton` bajo Liga/Equipo/Temporada en `ItemForm.tsx`. `pa_jugador` excluido estructuralmente (mapa de taxonomías controladas solo tiene `pa_liga`/`pa_equipo`/`pa_ano`). No se tocó `bridge.ts`, `client.ts`, `actions.ts` (guardado de item) ni categorías. typecheck/build (8/8)/lint/git diff --check PASS, secret scan CLEAN. Ningún término creado por el agente — pendiente test real de Pablo. Ver `docs/studio/STUDIO_CONTROLLED_TERM_CREATION_RESULT.md`.
