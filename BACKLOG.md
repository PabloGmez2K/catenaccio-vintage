...
- [ ] **S023E — CATEGORY_SELECTOR_IN_STUDIO** — **READY_FOR_PABLO_SQL_APPLY → READY_FOR_PABLO_CATEGORY_SELECTOR_TEST (2026-07-01, Opus 4.8).** Implementado el selector de categoría WC en el formulario, alimentado por la caché `wc_categories` (S023A): heurística por liga como default (`null` = Selecciones/Otros Clubs) + override manual (Leyendas/Nuevo). Nuevo helper `studio/lib/wc/category-cache.ts` centraliza la heurística (consumida por el form y el bridge). `bridge.ts` resuelve `categoryId` = override `football_shirt_details.categoria` o heurística; `categories[]` y `rank_math_primary_product_cat` comparten el mismo id (alineación garantizada). Persistencia: SQL **aditivo** `docs/studio/STUDIO_CATEGORY_SELECTOR_SCHEMA.sql` (2 columnas en `football_shirt_details`, `ADD COLUMN IF NOT EXISTS`) — **aplicar antes de usar el form**. Sin crear categorías Woo, sin publicar, DRAFT_ONLY intacto. typecheck/build/lint PASS. Gates PASS (DOMAIN_PRODUCT_MODELING, ACF_CONFIG, DATA_LAYER, NO_MICROPATCH). Pendiente: Pablo aplica SQL → test borrador con override Leyendas en WP Admin. Ver `docs/studio/STUDIO_CATEGORY_SELECTOR_RESULT.md`.
