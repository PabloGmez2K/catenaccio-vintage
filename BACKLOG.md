...
- [~] **S022C — STUDIO_WC_DRAFT_BRIDGE** — **FIX_BLOCKER_FIRST / TERM_ID_GAP (2026-06-28, Codex S022C.2).** Implementación técnica completa + FIX_BLOCKER_FIRST aplicado + S022C.1 (precio desde flujo SEO) PASS. Test real bloqueó correctamente antes de Woo con `equipo_term_id_required`; no se creó borrador. S022C.2 confirmó por GET Woo que los term IDs existen en taxonomías, pero no pudo identificar el `equipo_display` / `temporada_display` del item real porque Supabase local solo tiene anon key y RLS bloquea lectura directa. **Pendiente: Pablo devuelve equipo/temporada exactos del item que falló, o lectura manual de esos dos campos en Supabase; luego actualizar solo esas entradas en `wc-terms-mvp.ts`, Editar → Guardar item y reintentar 1 borrador.** Ver `docs/studio/STUDIO_WC_DRAFT_BRIDGE_RESULT.md`. Veredicto actual: `STOP`.
