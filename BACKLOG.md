...
- [~] **S022C — STUDIO_WC_DRAFT_BRIDGE** — **READY_FOR_PABLO_TERM_REWRITE_TEST (2026-06-28, Sonnet S022C.4).** Implementación técnica completa. S022C.3: ambos términos ausentes en WC → Pablo los creó manualmente (PSV Eindhoven=179, 2007-09=180). S022C.4: `wc-terms-mvp.ts` parcheado con ambos IDs; 2007-09 insertado en `buildSeasonOptions()` via splice. typecheck/build/lint PASS. **Pendiente (acción Pablo):** Abrir item → Editar → confirmar PSV/2007-09/Eredivisie → Guardar → volver a ficha → "Crear borrador en WooCommerce" 1 vez. Ver §17 en `docs/studio/STUDIO_WC_DRAFT_BRIDGE_RESULT.md`.
