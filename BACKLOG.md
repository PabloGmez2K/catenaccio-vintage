...
- [~] **S022C — STUDIO_WC_DRAFT_BRIDGE** — **READY_FOR_PABLO_WC_DRAFT_TEST (2026-06-28, Sonnet).** Implementación técnica completa + FIX_BLOCKER_FIRST aplicado: `formatWcRegularPrice()` helper correcto (entero → `"45"`, céntimos → `"44.99"`), validación de term IDs con regex numérico (equipo/temporada requeridos, liga/jugador opcionales), persistencia post-201 con check explícito del update (si falla → `WC_CREATED_SUPABASE_UPDATE_FAILED` con STOP_DO_NOT_RETRY, lifecycle failure no bloquea idempotencia). typecheck/build/lint PASS. **Pendiente: Pablo crea 1 borrador real con `.env.local` configurado y verifica en WP Admin.** Ver `docs/studio/STUDIO_WC_DRAFT_BRIDGE_RESULT.md`. Veredicto esperado tras prueba: `APPROVE_E2E_LOOP_PROVEN`.
