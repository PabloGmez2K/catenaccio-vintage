...
- [x] **S023B — TERM_CACHE_BACKED_OPTIONS** — **READY_FOR_PABLO_CACHE_BACKED_DRAFT_TEST (2026-06-30, Sonnet).** `studio/lib/wc/term-cache.ts` (nuevo) resuelve Liga/Equipo/Año contra `wc_terms` (Supabase, S023A). `actions.ts` y `bridge.ts` ya no leen `wc-terms-mvp.ts` como fuente de term IDs; el `.ts` queda marcado deprecado y conservado solo como presentación (labels/aliases/titleLabel/helpText). `getTermLabelById()` eliminado por quedar sin uso. typecheck/build/lint/git diff --check PASS, secret scan CLEAN. Sin llamadas a WooCommerce, sin términos creados, sin publicar. Ver `docs/studio/STUDIO_TERM_CACHE_BACKED_OPTIONS_RESULT.md`. Pendiente: test real de Pablo (borrador de equipo que ya existía en WC pero tenía `termId:''`).
