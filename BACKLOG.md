...
- [~] **S022B — STUDIO_AI_SUGGESTIONS_SHADOW** — **READY_FOR_PABLO_AI_SHADOW_TEST (2026-06-28, Sonnet).** Implementación completa: `@anthropic-ai/sdk`, contexto sanitizado, prompt `studio_s022b_v1`, server actions (generate/approve/reject/editAndApprove), `AiSuggestionsPanel` en ficha detalle, estilos. DOMAIN_PRODUCT_MODELING_GATE aplicado. typecheck/build/lint PASS. Pendiente: Pablo configura `ANTHROPIC_API_KEY` local, genera y aprueba una sugerencia, responde `PABLO_AI_SHADOW_OK`. S022C sigue BLOQUEADO hasta ese OK. Ver `docs/studio/STUDIO_AI_SUGGESTIONS_SHADOW_RESULT.md`. PATTERN-10 / DOMAIN_PRODUCT_MODELING_GATE aplicado en S022B.
