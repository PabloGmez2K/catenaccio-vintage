...
- [ ] **STUDIO_VISUAL_REVIEW_PABLO** — Pablo lanza Studio local (`cd studio && npm run dev`), hace login con magic link, verifica empty state + inventario. Acción manual, sin agente. Prerequisito de S022.
- [ ] **WORKSPACE_SEED_MANUAL** — Pablo ejecuta INSERT INTO workspaces manualmente en Supabase SQL Editor (ver runbook `STUDIO_MVP_LOCAL_RUNBOOK.md §6`). Acción manual. Prerequisito de S022 para ver datos.
- [ ] **S022 — STUDIO_CREATE_AND_PUBLISH** — formulario alta camiseta (inventory_items + football_shirt_details) + "Sugerir con Claude" (Anthropic API) + botón "Crear borrador en WC". (Sonnet/Codex)
