...
- [~] **S022C — STUDIO_WC_DRAFT_BRIDGE** — **READY_FOR_PABLO_WC_DRAFT_TEST (2026-06-28, Sonnet).** Implementación técnica completa + FIX_BLOCKER_FIRST aplicado + S022C.1 (precio desde flujo SEO): campo "Precio web / WooCommerce (€)" en ManualSeoPanel; `saveManualSeoContent` propaga precio a `inventory_items.precio_publicado_web`; panel muestra precio actual; validación server-side añadida. typecheck/build/lint PASS. **Pendiente: Pablo crea 1 borrador real con `.env.local` configurado y verifica en WP Admin.** Ver `docs/studio/STUDIO_WC_DRAFT_BRIDGE_RESULT.md`. Veredicto esperado tras prueba: `APPROVE_E2E_LOOP_PROVEN`.
