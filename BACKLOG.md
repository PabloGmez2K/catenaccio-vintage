...
- [ ] **OPUS_STRATEGY_REVIEW_BEFORE_S023_IMPLEMENTATION** — NOW / obligatorio antes de implementar S023. S022C queda técnicamente validado, pero la sesión expuso deuda de arquitectura de integración Woo/ACF: term IDs, `wc-terms-mvp.ts`, ACF companion keys, ACF Taxonomy `save_terms/load_terms`, `wp_term_relationships`, categories, stock, Rank Math e imágenes. Opus debe definir arquitectura target Studio/Woo/ACF, roadmap S023-S030, gates por sesión y qué NO implementar todavía. Ver `docs/studio/STUDIO_SUITE_BUILD_SYSTEM_LESSONS.md` + `docs/studio/STUDIO_OPUS_STRATEGY_BRIEF.md`.
- [ ] **S023 — STUDIO_WC_TAXONOMY_AND_CATEGORY_MANAGER** — Gestión propia desde Studio de atributos, términos y categorías WooCommerce. Incluye: (1) selector/creación/edición de categorías para Leyendas/Nuevo/Otros Clubs/Selecciones desde Studio; (2) create/sync términos `pa_liga`, `pa_equipo`, `pa_ano`, `pa_jugador` sin pasar por WP Admin; (3) read/sync bidireccional de términos; (4) reemplazar `studio/lib/wc-terms-mvp.ts` por sync real de términos/categorías para evitar mapa manual; (5) resolución de jugador (`PLAYER_TERM_RESOLUTION`, caso Rivaldo); (6) batch-PATCH de productos con payload v2.1 (incluyendo corrección productos 1854/1856 si procede). Ver `docs/studio/STUDIO_WC_DRAFT_PUBLISHABILITY_GAP_AUDIT.md §QUÉ DEBE QUEDAR PARA S023`.
- [ ] **S022D / S024 — STUDIO_VERCEL_DEPLOY_MINIMAL** — DIFERIDO hasta E2E probado en local. Deploy app probada, migrar env vars a Vercel server-side, configurar redirect URL Supabase, tratar postcss CVE en frontera pública. No de-riesga el puente (S020B ya probó acceso WC desde local) → no precede a Woo. (infra controlada)
