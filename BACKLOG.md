...
- [ ] **S023A — WC_TAXONOMY_CATEGORY_READ_SYNC** — ◄ PRIMER BLOQUE. Caché Supabase (`wc_taxonomies`/`wc_terms`/`wc_categories`) + sync **read-only** del vocabulario WC existente (attrs 4/5/6/7 + categorías). NO escribe en Woo, NO toca bridge/form/`wc-terms-mvp.ts`. Agente **Codex**. Gate: `DATA_LAYER_MAPPING`. Test Pablo: tras sync, Real Madrid→70 / FC Barcelona→170 sin editar `.ts`; counts caché == WC (21 equipos/18 temporadas/5 categorías). SQL aplicado manual por Pablo (patrón S020D). Cierre: `APPROVE_READY_FOR_S023B`.
