...
- [~] **S022C — STUDIO_WC_DRAFT_BRIDGE** — **READY_FOR_PABLO_TAXONOMY_DRAFT_TEST (2026-06-28, Codex S022C.8).** Hito E2E validado: borrador 1856 creado con payload v2.0; meta_data ACF correcto, pero ACF Taxonomy no mostraba Liga/Equipo/Año por falta de relaciones reales. S022C.8: payload v2.1 añade `attributes[]` complementario (`pa_liga` id 5, `pa_equipo` id 4, `pa_ano` id 7, `pa_jugador` id 6 si hay label seguro), `visible:false`, `variation:false`, snapshot ampliado. meta_data ACF conservado. typecheck/build/lint PASS. **Pendiente (acción Pablo):** crear nuevo item/borrador controlado y verificar en WP Admin que "Detalles del Producto" muestra Liga/Equipo/Año seleccionados. Ver `docs/studio/STUDIO_WC_DRAFT_BRIDGE_RESULT.md §S022C.8`.
- [ ] **S023 — STUDIO_WC_TAXONOMY_AND_CATEGORY_MANAGER** — Gestión propia desde Studio de atributos, términos y categorías WooCommerce. Incluye: (1) selector de categoría en UI para Leyendas/Nuevo/Otros/Selecciones; (2) create/sync términos pa_equipo/pa_liga/pa_ano/pa_jugador sin pasar por WP Admin; (3) read/sync bidireccional de términos; (4) reemplazar `studio/lib/wc-terms-mvp.ts` por sync real de términos/categorías; (5) batch-PATCH de productos con payload v2.1 (incluyendo corrección productos 1854/1856 si procede). Ver `docs/studio/STUDIO_WC_DRAFT_PUBLISHABILITY_GAP_AUDIT.md §QUÉ DEBE QUEDAR PARA S023`.
