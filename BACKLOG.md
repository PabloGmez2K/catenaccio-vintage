...
- [ ] **S025 — STUDIO OPERATIONS CONTROL** (fase, REORDER 2026-07-01, Opus `REORDER_TO_S025A_OPERATIONS_CONTROL`) — **imágenes DIFERIDAS a S026.** Antes de imágenes, Studio debe controlar su propio ciclo de vida. Verificado en código: hoy solo existe crear item / editar item (local) / crear borrador Woo (create-only, idempotente, DRAFT_ONLY); **no** hay archivar/borrar, transición de estados, update/PUT a Woo ni lectura del estado real de Woo. Fase descompuesta por clase de riesgo (read/local → Woo-write), una tarea por sesión:
  - [ ] **S025A — INVENTORY_CONSOLE_AND_WOO_STATE_READONLY** ◄ PRIMER BLOQUE (Codex, GET-only). Consola de inventario + lectura viva del estado real Woo por `wc_product_id`. Cero escrituras. Adelanta la parte read-only del antiguo S029.
  - [ ] **S025B — LOCAL_LIFECYCLE_AND_SOFT_ARCHIVE** (Sonnet, Supabase local). Transición de estados + soft-archive (`status='archivada'`, sin hard delete). Promueve `STUDIO_ARCHIVE_OR_DELETE_ITEM_ACTION`. Da el estado "dejar preparado".
  ...
  - Orden: A→B→C→D; E/F paralelizables. DRAFT_ONLY y publish manual intactos. Ver `docs/studio/STUDIO_POST_S024_ROADMAP_REORDER.md`.
- [ ] **S026–S031 — SUITE ROADMAP (reordenado +1)** — S026 imágenes (UX móvil, Sonnet+Antigravity), S027 stock operativo, S028 Rank Math/SEO, S029 landings (consume S025E), S030 drift full (Codex), S031 publicación controlada (habilita publish móvil). `STUDIO_MOBILE_PHOTO_FINISH_FLOW` = S025B + DEPLOY + S026 + S031 (capstone). Una tarea por sesión, gates por fase. Ver `docs/studio/STUDIO_POST_S024_ROADMAP_REORDER.md`, `docs/studio/STUDIO_ROADMAP_S023_S030.md §3`, gates `docs/studio/STUDIO_SESSION_GATES.md §7`.
