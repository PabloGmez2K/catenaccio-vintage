...
- [ ] **SUPABASE_SCHEMA_APPLY_MVP** — STOP_MANUAL_APPLY_REQUIRED en S020C. SQL canónico validado estáticamente; Codex no pudo aplicar sin pedir secretos porque no hay Supabase CLI/psql configurado. Pablo debe aplicar `docs/studio/STUDIO_SUPABASE_SCHEMA_MVP.sql` manualmente en Supabase SQL Editor y ejecutar `scripts/studio/verify_supabase_schema_mvp.sql`.
