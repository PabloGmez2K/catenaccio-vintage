...
- [ ] **SERVER_FILESYSTEM_READONLY_DISCOVERY** — **BLOCKED_WEBDAV_CONNECTION** (Sesión 010A, 2026-06-14). Puertos 2077/2078 bloqueados por firewall Raiola. Webdisk subdomain sin DNS. Ver `docs/operations/SERVER_FILESYSTEM_READONLY_DISCOVERY.md §5` para rutas de desbloqueo. **Desbloqueo (Ruta A):** Pablo pega 4 ficheros desde cPanel File Manager → `functions.php`, `style.css`, `filtro-camisetas-pro/main.php`, `off-canvas-menu/main.php`. 20-30 min. Script `scripts/filesystem/webdav_probe.py` listo para reintentar si se resuelve acceso WebDAV.
- [ ] **A0_MIGRATION_PLAN** — plan técnico PHP child theme para P1-A (header), P1-B (producto individual), P1-C (archivo productos). Bloqueado hasta SERVER_FILESYSTEM_READONLY_DISCOVERY. Próxima sesión post-desbloqueo.
