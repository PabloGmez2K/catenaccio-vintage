...
6. ~~**Sesión 010A — SERVER_FILESYSTEM_READONLY_DISCOVERY:** intentado vía WebDAV — **BLOCKED** (puertos 2077/2078 bloqueados por Raiola). Ver `SERVER_FILESYSTEM_READONLY_DISCOVERY.md §5`.~~
7. **Desbloqueo (Ruta A — más rápida):** Pablo abre cPanel File Manager y pega 4 ficheros: `functions.php`, `style.css`, plugin Filtro Camisetas Pro, plugin Off-Canvas Menu. 20-30 min. Sin agente activo. Luego sesión 010B para A0_MIGRATION_PLAN.
8. **Acción Pablo (sin agente, ya posible):** CARRITO_MICUENTA_QUICKWIN — reemplazar widgets Pro en Carrito y Mi Cuenta por shortcodes `[woocommerce_cart]` y `[woocommerce_my_account]` en WP Admin → Elementor. 10 min.
